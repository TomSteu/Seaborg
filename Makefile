...
all: wstp_connection.o vala
	...
	$(CC) $(CCFLAGS) `pkg-config --cflags gtk+-3.0 gtksourceview-3.0 glib-2.0 ` -c ./build/src/parameter.c -o ./build/parameter.o `pkg-config --libs gtk+-3.0 gtksourceview-3.0 glib-2.0`
	$(CC) $(CCFLAGS) -L$(WSTPDIR)  ./build/cell.o ./build/gui.o ./build/string.o ./build/parameter.o ./build/wstp_connection.o -o ./bin/seaborg `pkg-config --libs gtk+-3.0 gtksourceview-3.0 glib-2.0` -l$(WSTPLIB)
	
.PHONY: vala
vala:
	valac --thread ./src/gui.vala ./src/cell.vala ./src/string.vala ./src/parameter.vala -d ./build/ -C --pkg gtk+-3.0 --pkg gtksourceview-3.0
